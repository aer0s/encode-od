...
def main(source, output, force, no_logging, eject_disc, notify, email_sender,
    ...
    if not os.path.isdir(groupP) or force:
        if event_start:
            shell(event_start, shell=True, standalone=True)
        ...
        try:
            ...
            try:
                ...
                handbrake('/dev/sr%d' % source, os.path.join(groupP, '%s.mkv' %
                                                             title))
            except Exception as Err:
                log(
                ...
                exit()
        if event_disc_done:
            shell(event_disc_done, shell=True, standalone=True)
        if eject_disc:
            eject(source)
        ...
        if event_done:
            shell(event_done, shell=True, standalone=True)
    else:
        log(' '.join([
            'This od has been encoded already. If you woul like to do it',
            'again plese use --force option.'
            ]), email_temp)
...
def convert_subtitles(output, title):
    ...
    if sfiles['id']:
        ...
        c = "mkvmerge -o %(mkvCopy)s --subtitle-tracks %(id)d %(mkv)s %(srt)s"
        shell([c % sfiles], shell=True)
        print('Cleaning up ...')
        ...
        os.rename(os.path.join(output, '%s-working.mkv' % title),
                  os.path.join(output, '%s.mkv' % title))
        print('done.')
    else:
        print('Well damn it!')
